...
const ToxicSubstancesAddEmployeePage = () => {
  ...
  const toxicSubstanceAddEmployeeBreadCrumbs = [
    {
        name: "HR Dashboard",
        link: "/dashboard",
    },
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      const result = await dispatch(addNewClientEmpoyeeAPI({data:payload,clientId:linkableId}));
      if (result?.error) {
  ...
  return (
    <AddEmployeeStyled>
      <div className="!px-[39px] !py-[48.7px] bg-white rounded-lg">
        <CommonBreadCrumbs
          className="mb-2"
          items={toxicSubstanceAddEmployeeBreadCrumbs}
        />
              ...
              >
                <Radio.Group>
                  <div className="grid grid-cols-3 gap-[20px] h-[39px]">
                    <Radio.Button
                      value="male"
                      className="px-3 py-1 rounded-full gender-style"
                    >
                      Male
                    </Radio.Button>
                    <Radio.Button
...
export default ToxicSubstancesAddEmployeePage;
