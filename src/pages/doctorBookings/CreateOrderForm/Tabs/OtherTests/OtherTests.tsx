...
import { ConsultType, FormData, ValidationErrors } from "../../CreateOrder";
...
const OtherTests = ({
  ...
  const [vendors, setVendors] = useState<any[]>([defaultVendor]);
  const [selectedVendor, setSelectedVendor] = useState<any>(defaultVendor);
  ...
  }, [testData.selectedDate, dispatch, selectedVendor]);
  ...
  const handleTestSelect = (selectedOptions: any) => {
    ...
    if (validationErrors?.vendor || validationErrors?.test) {
      setValidationErrors((pre: any) => ({
        ...pre,
        vendor: selectedVendor?.id ? null : pre.vendor,
  ...
  const updateTestsFormData = (key: string, value: any) => {
    setFormData((pre: any) => ({
      ...pre,
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div>
              ...
              {validationErrors.vendor && (
                <p className="!m-0 text-red-500">{validationErrors.vendor}</p>
              )}
            </div>
          </div>
